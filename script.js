...
function parcela(preco, taxa, quant) {
    if (quant === 0) return preco; // Evita divisão por zero
...
function calcularValores() {
    ...
    if (quantBalao < 0 || quantBalao > 23) {
        ...
        quantBalao = Math.min(Math.max(quantBalao, 0), 23);
    }
    if (quantParcela > 276) {
    ...
    if (quantBalao > periodoPagamentoAno) {
        alert(`O período digitado é de ${periodoPagamentoAno} ano${plural}, portanto ajuste a quantidade de balões.`);
    ...
    if (porcentagemBalao + porcentagemEntrada > 100) {
        alert("A soma das porcentagens não pode ser maior que 100%.");
        porcentagemBalao = 61;
        porcentagemEntrada = 10;
    }

    // Cálculo dos valores
    valorTotal *= (100 - desconto) / 100;
    ...
    const valorBalao = parcela(valorTotal * (porcentagemBalao / 100), 8 / 100, quantBalao || 1);
    const taxaMensal = ((1 + (8 / 100)) ** (1 / 12)) - 1;
    const valorParcela = parcela(valorTotal * (porcentagemParcela / 100), taxaMensal, quantParcela);
    const valorParcela2 = parcela(valorTotal * ((porcentagemBalao + porcentagemParcela) / 100), taxaMensal, quantParcela);
